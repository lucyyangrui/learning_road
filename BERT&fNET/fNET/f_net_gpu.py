...
class FFTNet(nn.Module):
    ...
    def forward(self, x):
        out = self.fft.fft2(x, dim=(-1, -2)).real
        out = self.norm(out)
        return out
...
class EncoerLayer(nn.Module):
    ...
    def forward(self, enc_inputs):
        # residule = enc_inputs
        enc_outputs = self.enc_fft(enc_inputs)
...
class BERT(nn.Module):
    ...
    def forward(self, input_ids, segment_ids, masked_pos):
        ...
        # masked_pos 的size = batch_size * max_pred * d_model
...
logits_lm, logits_clsf = model(torch.LongTensor([input_ids]).to(device),
                 torch.LongTensor([segment_ids]).to(device), torch.LongTensor([masked_pos]).to(device))
logits_lm = logits_lm.data.max(2)[1][0].data
print('masked tokens list : ',[pos for pos in masked_tokens if pos != 0])
print('predict masked tokens list : ',[pos for pos in logits_lm if pos != 0])

logits_clsf = logits_clsf.data.max(1)[1].data
